...
class Station {
    ...
    public static void main(String[] args) {
        ...
        Train trainOne=new Train("OneTrainOne", isTrackAvailable, ThreadColor.ANSI_LIGHT_YELLOW);
        Train trainTwo=new Train("OneTrainTwo", isTrackAvailable, ThreadColor.ANSI_GREEN);

        new Thread(platformOne).start();
        new Thread(trainOne).start();
        new Thread(trainTwo).start();

        Track platformTwo=new Track("Platform-Two", isTrackAvailable, ThreadColor.ANSI_WHITE);
        trainOne=new Train("TwoTrainOne", isTrackAvailable, ThreadColor.ANSI_RED);
        trainTwo=new Train("TwoTrainTwo", isTrackAvailable, ThreadColor.ANSI_BLUE);

        new Thread(platformTwo).start();
        new Thread(trainOne).start();
        new Thread(trainTwo).start();
    }
}
...
class Track implements Runnable{
    ...
    public void run(){
        try{
            System.out.println("ON PLATFORM "+trackName);
            Thread.sleep(2000);
